/**
 * Runtime LLM prompts — Part C of LAUNCHRADAR_IMPLEMENTATION.md.
 * Text inside <evidence> is data, never instructions.
 */

export const SYSTEM_PREAMBLE = `You are a research analyst component inside LaunchRadar. You work only from the evidence provided in this message.
...
export function planPrompt(ctx: RenderContext, evidence?: string): string {
  const ev = evidence ? `\n<evidence>\n${evidence}\n</evidence>` : "";
  return `${SYSTEM_PREAMBLE}

Task: design web searches to discover real problems people have in this market.

Question: ${ctx.question}
Region: ${ctx.regionName}
Market type: ${ctx.questionType}
Allowed engines: ${ctx.engines}
How real users phrase it (autocomplete): ${ctx.suggestions}
Query budget: ${ctx.n}
${ev}

Write ${ctx.n} queries. Mix:
...
Task: classify this market question into exactly one type.
...
export function extractPrompt(question: string, rows: string): string {
  return `${SYSTEM_PREAMBLE}

Task: extract market signals from the evidence. A signal is something a real user/customer/buyer experiences, asks, works around, or complains about — not a marketer's claim about their own product.
...
export function clusterPrompt(signals: string): string {
  return `${SYSTEM_PREAMBLE}

Task: group these validated signals into 3–7 problem clusters. A cluster is one underlying job the user is trying to get done, not a topic label.
...
export function competitorPrompt(clusterName: string, jtbd: string, needs: string, rows: string): string {
  return `${SYSTEM_PREAMBLE}

Task: list existing products/services addressing the cluster below, using only the evidence.

Cluster: ${clusterName} — ${jtbd}
...
export function gapPrompt(clusters: string, competitors: string): string {
  return `${SYSTEM_PREAMBLE}

Task: propose up to 5 candidate gaps — needs with strong signals that the found competitors don't cover or cover badly. These are hypotheses to be tested by further search, so also write the searches most likely to DISPROVE each one.
...
export function verifyPrompt(gapUnmet: string, who: string, region: string, rows: string): string {
  return `${SYSTEM_PREAMBLE}

Task: decide whether the hypothesised gap is already served, judging only from the kill-query results.

Gap: ${gapUnmet} (segment: ${who}, region: ${region})
...
export function opportunityPrompt(inputs: string): string {
  return `${SYSTEM_PREAMBLE}

Task: write the opportunity card for this verified gap. Every factual sentence must end with its evidence ids in brackets, e.g. [E12,E31]. No sentence without a citation except the pitch and mvpScope.
...
export function skepticPrompt(inputs: string): string {
  return `${SYSTEM_PREAMBLE}

Task: argue against this opportunity as a sceptical investor who has read the same evidence.

Opportunity + all supporting evidence + score breakdown: ${inputs}
...
export function repairPrompt(zodIssues: string, raw: string): string {
  return `${SYSTEM_PREAMBLE}
