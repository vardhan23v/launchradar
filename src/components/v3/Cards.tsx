...
import { ConfidenceBadge, ExampleBadge, GapBadge, Monogram, ScoreDial, ScoreRadar, ShortlistButton, SourcesPopover, cx } from "./parts";
...
export interface CardProps {
  ...
  onOpen: () => void;
  /** hide the research-question line (the run page, where every card shares one question) */
  hideOrigin?: boolean;
}
...
function Origin({ item, now }: { item: FeedItem; now: number }) {
  return (
    <p className="flex min-w-0 items-center gap-2 text-xs text-zinc-400">
      ...
      <span className="sr-only">{regionLabel(item.region)}:</span>
      ...
      <span className="shrink-0">{item.demo ? "Recorded example" : item.superseded ? "Earlier run" : relativeDay(item.createdAt, now)}</span>
    </p>
  );
}

/** #1 of the current list: glowing border, the sub-score pentagon as its preview. */
export function FeaturedCard({ item, shortlisted, onToggleShortlist, onOpen, scopeLabel, hideOrigin }: Omit<CardProps, "rank" | "now"> & { scopeLabel: string }) {
  return (
    <section aria-labelledby={`lr3-featured-${item.key}`} className="lr3-appear relative" data-feed-key={item.key}>
      ...
      <div className="relative rounded-2xl bg-linear-to-br from-indigo-400/70 via-sky-400/30 to-emerald-400/60 p-px shadow-[0_0_60px_-20px_rgba(99,102,241,0.65)]">
        <div className="relative rounded-[15px] bg-zinc-950/90 backdrop-blur-xl">
          <div aria-hidden="true" className="pointer-events-none absolute inset-0 rounded-[15px] bg-[radial-gradient(ellipse_at_top_right,rgba(99,102,241,0.18),transparent_55%)]" />
          <div className="relative grid gap-6 p-5 sm:p-7 md:grid-cols-[minmax(0,1fr)_auto] md:gap-8">
            <div className="flex min-w-0 flex-col gap-4">
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="min-w-0">
                  <h2 id={`lr3-featured-${item.key}`} className="text-balance text-2xl font-semibold tracking-tight text-white [overflow-wrap:anywhere] sm:text-3xl">
                    <button type="button" onClick={onOpen} data-open="" className="rounded-md text-left transition-colors hover:text-indigo-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400/70">
                      {item.title}
                    </button>
                  </h2>
              ...
              <dl className={cx("grid gap-x-6 gap-y-2 text-sm", !hideOrigin && "sm:grid-cols-2")}>
                <div className="min-w-0">
                  <dt className="text-xs text-zinc-400">For</dt>
                  <dd className="text-zinc-200">{item.opportunity.target}</dd>
                </div>
                {!hideOrigin && (
                  <div className="min-w-0">
                    <dt className="text-xs text-zinc-400">Found by researching</dt>
              ...
              <div className="mt-auto flex flex-wrap items-center gap-2 pt-1">
                ...
                <SourcesPopover sources={item.sources} title={item.title} onSeeAll={onOpen} />
              </div>
            </div>
            ...
            <div className="flex items-center justify-center gap-5 rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 md:flex-col md:justify-between">
              <div className="flex flex-col items-center gap-1.5 md:order-2">
                ...
                <span aria-hidden="true" className="text-[11px] uppercase tracking-wider text-zinc-400">
                  Score
                </span>
              </div>
...
export function GridCard({ item, rank, now, shortlisted, onToggleShortlist, onOpen, hideOrigin }: CardProps) {
  return (
    <article
      ...
      className={cx(
        ...
        "focus-within:border-indigo-500/50 has-[[aria-expanded=true]]:z-30",
      )}
    >
      ...
      <div className="mt-auto flex flex-wrap items-center justify-between gap-x-3 gap-y-3 border-t border-white/[0.06] pt-4">
        <div className="flex flex-wrap gap-1.5">
          {item.demo && hideOrigin && <ExampleBadge />}
          <GapBadge status={item.gapStatus} />
          <ConfidenceBadge value={item.confidence} />
        </div>
        <div className="ml-auto flex shrink-0 items-center gap-2">
          <SourcesPopover sources={item.sources} title={item.title} onSeeAll={onOpen} align="end" />
          <ShortlistButton active={shortlisted} onToggle={onToggleShortlist} title={item.title} compact />
        </div>
      </div>
    </article>
  );
...
export function ListRow({ item, rank, now, shortlisted, onToggleShortlist, onOpen, hideOrigin }: CardProps) {
  return (
    <li
      data-feed-key={item.key}
      className={cx(
        "lr3-appear group relative flex items-center gap-3 px-3 py-3 sm:gap-4 sm:px-4",
        "transition-colors duration-200 hover:bg-white/[0.03] focus-within:bg-white/[0.03] has-[[aria-expanded=true]]:z-30",
      )}
    >
      <span className="hidden w-6 shrink-0 text-right font-mono text-xs tabular-nums text-zinc-400 sm:block">
      ...
      <div className="min-w-[8rem] flex-1">
        <h3 className="truncate text-sm font-semibold text-zinc-100">
          <button type="button" onClick={onOpen} data-open="" className="text-left after:absolute after:inset-0 focus-visible:outline-none focus-visible:after:ring-2 focus-visible:after:ring-inset focus-visible:after:ring-indigo-400/70">
            {item.title}
          </button>
        </h3>
      ...
      {!hideOrigin && (
        <div className="hidden min-w-0 shrink md:block md:w-40 xl:w-56">
          <Origin item={item} now={now} />
        </div>
      )}
      <ScoreDial score={item.score} size="sm" className="pointer-events-none" />
      <div className="hidden sm:block">
        <SourcesPopover sources={item.sources} title={item.title} onSeeAll={onOpen} align="end" compact />
      </div>
      <ShortlistButton active={shortlisted} onToggle={onToggleShortlist} title={item.title} compact />
    </li>
  );
